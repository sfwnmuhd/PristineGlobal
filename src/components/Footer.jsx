...
const Footer = () => {
  ...
  return <footer className="w-full bg-white">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3 md:gap-8 py-10 max-sm:max-w-sm max-sm:mx-auto gap-y-8">
          ...
          {Object.entries(footerLinks).map(([title, links]) => <div key={title} className="lg:mx-auto text-left">
              <h4 className="text-lg text-black font-medium mb-7 capitalize">
                {title}
              </h4>
              <ul className="text-sm transition-all duration-500">
                {links.map((link, index) => <li key={index} className={index === links.length - 1 ? "" : "mb-6"}>
                    <a href={link.href} className="text-black hover:text-indigo-600">
                      {link.name}
                    </a>
                  </li>)}
              </ul>
