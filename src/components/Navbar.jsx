            ...
            if (item.type === 'route') {
              return (
                <Link
                  ...
                  to={item.href}
                  onClick={() => closeMenu()}
                  className={`${(isAboutPage || isLocationPage) && !isScrolled ? 'text-gray-900' : 'text-gray-700'} hover:text-[#0b3b5c] transition-colors font-medium text-sm xl:text-base cursor-pointer`}
                >
                  {item.name}
                </Link>
              )
